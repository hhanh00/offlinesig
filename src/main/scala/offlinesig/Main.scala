...
object Main {
  ...
  def toBitcoinLeaf(priv: PrivKeyExt) = priv.toPrivChild(0x8000002C).toPrivChild(0x80000000 + Bitcoin.addressPrefix).toPrivChild(0x80000000)
  ...
  def main(args: Array[String]) {
    val command = args.lift(0)
    command map {
      case "sign" => signTx(args(1), args(2), args.lift(3) getOrElse "")
      case "mpk" => showMpk(args(1), args.lift(2) getOrElse "")
      case "seed" => createNewSeed(args.lift(1))
      case "receive" => showAddresses(args(1), false, args(2).toInt)
      case "change" => showAddresses(args(1), true, args(2).toInt)
      case "prepare" => prepareTx(args(1))
      case "make" => makeTx(args(1), args(2))
    } getOrElse {
    ...
    def showAddresses(mpkFile: String, change: Boolean, count: Int) = {
      ...
      val addresses = Stream.from(0).map(i => addrStream.toPublicChild(i).get).map(p => Bitcoin.toAddress(p.point, Bitcoin.addressPrefix))
